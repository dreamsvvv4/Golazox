...
class App(tb.Window):
    def __init__(self):
        ...
        self.title("UltraKibanaDownloader Premium")
    ...
    def __init__(self):
        ...
        help_menu.add_command(label="About", command=lambda: messagebox.showinfo("About", "UltraKibanaDownloader Premium\nModem tools GUI"))
        ...
        logo_label = ttk.Label(top_frame, text="🟦 UltraKibanaDownloader Premium", font=("Segoe UI", 16, "bold"), foreground="#00BFFF")
    ...
    def __init__(self):
        ...
        self.title('Ultra Protom Sequence')
    ...
    def open_bulk_dialog(self):
        ...
        def on_submit():
            ...
            if not raw:
                ...
                return
            # Extract numbers from CSV or lines
            lines = [l.strip() for l in raw.splitlines() if l.strip()]
            installs = []
            for l in lines:
                # If CSV, take first cell
                if ',' in l:
                    installs.append(l.split(',')[0].strip())
                else:
                    installs.append(l)
            dlg.destroy()
            op = self.operacion_var.get()
        ...
        def task_for_install(inst_id: str):
            ...
            try:
                ...
                if operation == 'ChangeStatus':
                    ...
                    args = [str(tinfo), str(inst_id), str(params.get('Country', 'ES')), '1']
                    script = os.path.join(os.path.dirname(__file__), 'ChangeStatusOfTransmissions.py')
                    ...
                    env = os.environ.copy()
                    env['REMOTEARM_CODINSTALACION'] = str(inst_id)
                    env['REMOTEARM_CODPAIS'] = params.get('codPais', params.get('codPais', 'ESP'))
                    env['REMOTEARM_ORDERID'] = params.get('orderid', 'RemoteArm')
                    env['REMOTEARM_USERID'] = params.get('userId', '00')
                    env['REMOTEARM_ARMMODE'] = params.get('armMode', params.get('armMode', '23'))
                    script = os.path.join(os.path.dirname(__file__), 'RemoteArm.py')
                    proc = subprocess.run([sys.executable, script], capture_output=True, text=True, env=env)
                    out = proc.stdout + proc.stderr
                    status = 'ok' if proc.returncode == 0 else 'error'
                    return (inst_id, status, out)
                    ...
                    env = os.environ.copy()
                    env['REMOTEDISARM_CODINSTALACION'] = str(inst_id)
                    env['REMOTEDISARM_CODPAIS'] = params.get('codPais', 'ESP')
                    env['REMOTEDISARM_ORDERID'] = params.get('orderid', 'RemoteDisarm')
                    env['REMOTEDISARM_USERID'] = params.get('userId', '00')
                    env['REMOTEDISARM_ARMMODE'] = params.get('armMode', '00')
                    script = os.path.join(os.path.dirname(__file__), 'RemoteDisarm.py')
                    proc = subprocess.run([sys.executable, script], capture_output=True, text=True, env=env)
                    out = proc.stdout + proc.stderr
                    status = 'ok' if proc.returncode == 0 else 'error'
                    return (inst_id, status, out)
                    ...
                    env = os.environ.copy()
                    env['REMOTEREBOOT_CODINSTALACION'] = str(inst_id)
                    env['REMOTEREBOOT_CODPAIS'] = params.get('codPais', 'ESP')
                    env['REMOTEREBOOT_ORDERID'] = params.get('orderid', 'ResetPanelOrDevice')
                    env['REMOTEREBOOT_USERID'] = params.get('userId', '00')
                    env['REMOTEREBOOT_DEVICEID'] = params.get('deviceId', '01')
                    env['REMOTEREBOOT_DEVICETYPE'] = params.get('deviceType', '106')
                    script = os.path.join(os.path.dirname(__file__), 'RemoteReboot.py')
                    proc = subprocess.run([sys.executable, script], capture_output=True, text=True, env=env)
                    out = proc.stdout + proc.stderr
                    status = 'ok' if proc.returncode == 0 else 'error'
                    return (inst_id, status, out)
                    ...
                    env = os.environ.copy()
                    env['MUR_INSTALLATION_NUMBER'] = str(inst_id)
                    env['MUR_COUNTRY'] = str(params.get('Country', 'ES'))
                    env['MUR_NUMBEROFPICTURE'] = str(params.get('numberOfPicture', '1'))
                    env['MUR_DEVICEID'] = str(params.get('deviceId', '06'))
                    env['MUR_CU_VERSION'] = '2'
                    env['MUR_DEVICETYPE'] = str(params.get('deviceType', '106'))
                    env['MUR_MEDIATYPE'] = str(params.get('media_type', '1'))
                    env['MUR_RESOLUTIONFORMAT'] = str(params.get('resolution', '1'))
                    script = os.path.join(os.path.dirname(__file__), 'MediaUserRequest.py')
                    proc = subprocess.run([sys.executable, script, '1'], capture_output=True, text=True, env=env)
                    out = proc.stdout + proc.stderr
                    status = 'ok' if proc.returncode == 0 else 'error'
                    return (inst_id, status, out)
                elif operation == 'DoorLock':
                    env = os.environ.copy()
                    env['LUD_PAIS'] = params.get('pais', 'ESP')
                    env['LUD_IDINSTALACION'] = str(inst_id)
                    env['LUD_DEVICETYPE'] = params.get('deviceType', '163')
                    env['LUD_DEVICEID'] = params.get('deviceId', '01')
                    env['LUD_LOCK'] = params.get('lock', '1')
                    env['LUD_RESERVED'] = params.get('reserved', '0000000000000000')
                    script = os.path.join(os.path.dirname(__file__), 'LockUnlockDoorlock.py')
                    proc = subprocess.run([sys.executable, script], capture_output=True, text=True, env=env)
                    out = proc.stdout + proc.stderr
                    status = 'ok' if proc.returncode == 0 else 'error'
                    return (inst_id, status, out)
