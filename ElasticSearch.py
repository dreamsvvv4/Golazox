...
class ElasticSearch:
    ...
    def getHitsAround(self, dateAround, services):
        ...
        if requestedSize > 10000:
            ...
            return self.getHitsWithScroll(jsData, requestedSize)
        else:
            ...
            return self.parseHits(response)
    ...
    def getHitsWithScroll(self, jsData, maxResults):
        ...
        actualLimit = min(totalFound, maxResults)
        ...
        print("PROGRESS: Retrieved %s/%s logs (%.1f%%)" % (len(all_logs), actualLimit, (len(all_logs) / actualLimit) * 100))
        ...
        while len(response['hits']['hits']) > 0 and len(all_logs) < maxResults:
            ...
            # Calculate percentage and show progress
            percentage = (len(all_logs) / actualLimit) * 100
            print("PROGRESS: Retrieved %s/%s logs (%.1f%%)" % (len(all_logs), actualLimit, percentage))
